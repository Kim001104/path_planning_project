"""PID + 2D KF demo — true vs measured vs KF estimate, closed-loop (plotly).

실행 전 `closed_loop_kf.py` 의 `# TODO` 를 구현해야 동작합니다.
...
def main() -> None:
    ...
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                        subplot_titles=("position (true / measured / KF estimate)",
                                        "control u"),
    ...
    fig.add_trace(go.Scatter(x=t, y=y_estimate, mode="lines", name="KF estimate",
    ...
    fig.update_layout(
        ...
        title="PID + 2D KF — closed-loop with noisy measurement (R=0.25, disturbance=0.1)",
    )
    fig.show()
